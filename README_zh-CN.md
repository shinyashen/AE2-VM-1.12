...
- **净放大合成环**:互为投入产出、每轮净增产出的环形配方自动检测、代数求解至物料最小轮数并折叠为单一计划;副产物中间键、环外原料(缺口沿其自身样板链递归下探)、副产物直接下单、跨环耦合等形态全部覆盖。AE2UEL 的合成 CPU 对最终产物即产即交、从不回流喂环,因此直接下单环自身的净增益产物时,全额毛耗如实计入启动垫资;而增益经中间物流转的环形态可从极小垫资自举
