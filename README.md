...
- **Amplifying crafting rings** — circular recipes that net-grow per round are detected, solved to the material-minimal counts and folded into one plan; byproduct intermediates, external inputs (shortfalls recursed down their own producer chains), byproduct-rooted orders and coupled ring chains are all covered. AE2UEL CPUs deliver the final output the moment it is crafted — it never returns to feed the loop — so ordering the loop's own net-gain product honestly costs the full input draw as startup capital, while rings whose gain flows through intermediates bootstrap from minimal capital

  **Experimental in 1.2.0** — the ring plans are now billed so a real crafting CPU can execute them (the delivery is crafted, and every ring member's startup draw is charged), but the family ships off by default: enable `ringSolverEnabled` in the config to opt in. The faithful simulator runs every ring scenario to completion; live validation is still ongoing.
